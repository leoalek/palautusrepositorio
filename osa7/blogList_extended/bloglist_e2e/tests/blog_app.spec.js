    ...
    describe("and multiple blogs exist", () => {
      ...
      test("blogs are ordered by likes", async ({ page }) => {
        ...
        const blogTexts = await Promise.all(
          blogDivs.map((div) => div.innerText())
        );

        expect(blogTexts[0]).toContain("blog2 by Ted Tester");
        expect(blogTexts[1]).toContain("blog3 by Ted Tester");
        expect(blogTexts[2]).toContain("blog1 by Ted Tester");
      });
    });
  });
});
